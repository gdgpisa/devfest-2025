---
title: Announcing Our Amazing Speakers!
date: 2026-03-16
---

## Announcing Our Amazing Speakers! 📢

Hello everyone! 🥳

We know you've been eagerly awaiting this announcement, and today, we're thrilled to unveil some amazing speakers 🎉 for the upcoming DevFest Pisa 2026!

We can't wait to share the full agenda with you, but we're still finalizing some last details, so please stay tuned!

#### The speakers 🎤

#### We're happy to welcome on stage the following speakers and sessions:

<!-- generated with "./src/assets/sessionize/generate_speakers_list.py" -->
<!-- START -->

- **Francesco Sciuti & Emiliano Pisu** — CEO @ Devmy - Google Developer Expert, Microsoft MVP, Google Certified Developer & AWS UG Lead & Senior Design Engineer, Sensei & Co-Host @ DevDojoIT

    **_60fps o niente: Rendering Pipeline a suon di esempi pratici!_** - talk language 🇮🇹

- **Daniela Petruzalek** — Developer Relations Engineer at Google

    **_Advanced Techniques for Building Agents with ADK and Vertex AI_** - talk language 🇬🇧

- **Gregory Pardini** — Tech Lead @ Rosso

    **_Agenti in produzione: come sbloccare il potenziale in sicurezza_** - talk language 🇮🇹

- **Daniela Petruzalek** — Developer Relations Engineer at Google

    **_Build a photo restoration app using Genkit Go and Nano Banana Pro_** - talk language 🇬🇧

- **Mohamed Mortadha Manai** — Senior AI Engineer |AI Expert| AI Engineer @Netflix | PHD candidate in xAI | International AI Speaker

    **_Building Agentic AI Workflows with xAI on Vertex AI: From Code to Action_** - talk language 🇬🇧

- **Luca Del Puppo** — Nearform\_, Senior Software Engineer

    **_Costruiamo agenti con ADK-js_** - talk language 🇮🇹

- **Michel Murabito & Graziano Casto** — CNCF Ambassador & Tech Evangelist @Fractal Cloud & Developer Relations Engineer @ Akamas | Tech Lead @ CNCF TAG DevEx | Kubernetes v1.35 Comms Lead

    **_Da Zero a Cento: Costruire un’Infrastruttura che si Auto-Ottimizza_** - talk language 🇮🇹

- **Matthias Geisler** — Senior Software Engineer@Airalo

    **_Design patterns 101 with Kotlin_** - talk language 🇬🇧

- **Lorenzo Spiridioni** — Software Engineer @ RoxPay

    **_DevBox: Nix made easy_** - talk language 🇬🇧

- **Michela Bertaina** — Head of Community & Growth @ Codemotion| Partnership Team Lead @ TEDx Cuneo | WTM Ambassador | GDG Bari Lead | Tourism Councilor @ Cissone

    **_Developer Mezzosangue: Padroneggiare l'Alchimia dell'IA e del Personal Branding_** - talk language 🇮🇹

- **Valeria Salis** — Tech Sustainability Consultant, Speaker, Host @ Green Tech Tales

    **_From shy contributor to (kinda) confident community leader_** - talk language 🇬🇧

- **Nicola Corti** — Android GDE - Android @ React

    **_Gotta Build 'Em All: Android Pokedex 10 Years Later_** - talk language 🇬🇧

- **Chiara Corrado & Federica Merenda & Pinelopi Troullinou & Luciana Bertoncini** — Senior Software Engineer - Green Software Italia co-founder - WTM & She Tech Ambassador - Donne 4.0 member & Post-doc Researcher, Feminist AI and Ethics of Technology & AI Governance & Responsible Innovation Consultant & AI Process Eng @ Dumarey Flowmotion Technologies, Donne 4.0 Member

    **_Hack the Job Title: Il lavoro reale dietro i titoli tech_** - talk language 🇮🇹

- **Giacomo Sergio** — CEO@Wondersys

    **_IA Generativa e Inclusione: Progettare l'Autonomia Oltre i Bias di Performance_** - talk language 🇮🇹

- **Tiffany De Simone & Valentina Giuffre' Coco** — Full Stack Developer & Full Stack Developer presso TD Group Italia S.r.l.

    **_Il bug non era nel codice: era nel team!_** - talk language 🇮🇹

- **Jacopo Martellini** — Software Architect @ AuraSensae

    **_Il miglior Copilot è umano_** - talk language 🇮🇹🇬🇧

- **Federico Paolinelli** — Senior Principal Software Engineer at Red Hat

    **_Kubernetes Networking Demystified: From Pod IPs to Loadbalancers_** - talk language 🇬🇧

- **Juna Salviati** — GDG Roma

    **_L'essenziale è invisibile agli occhi (ma non a Python): un’introduzione alla steganografia_** - talk language 🇮🇹

- **Pietro Tempesti & Franco Riformato** — Software Engineer @NTT DATA Italy & Advanced Software Engineer @ NTT DATA

    **_La Context Engineering come l'arma finale di ogni sviluppatore_** - talk language 🇬🇧

- **Diego Giorgini** — Head of AI @ AI Technologies

    **_LLM Steering: progettare 'stati mentali' per modelli linguistici_** - talk language 🇮🇹

- **Irene Burresi** — AI Team Leader @ Value Partners

    **_LLM Testing 101: qualità e metriche di valutazione per sistemi GenAI_** - talk language 🇮🇹

- **Camillo Bucciarelli** — Technical manager @ Sopra Steria

    **_Mettiti l’LLM in tasca: AI generativa offline su smartphone con Flutter_** - talk language 🇮🇹

- **Fabian Greavu** — GenAI Engineer @ Gear 57

    **_Oltre i Chatbot: OpenClaw, Micro-Agenti e l'Ecosistema IA Connesso_** - talk language 🇮🇹

- **Donatella Pasquale & Matilde Petrucci** — Mobile app developer @ Garmin Italy Technologies SRL & Software Engineer @ Garmin Italy Technologies SRL

    **_One Feature, Many Apps: Building Without Repeating Yourself_** - talk language 🇬🇧

- **Monja Da Riva** — counselor, formatrice e speaker

    **_Pattern Recognition: identificare il codice invisibile dei non detti_** - talk language 🇮🇹

- **Luigi De Bianchi** — Senior backend software engineer at Klarna with a kink for software testing and everything related to non frontend technologies. Passionate maker and curios pastry chef in the free time.

    **_Senior Engineer ≠ More Code: A Git History Tell-All_** - talk language 🇮🇹

- **Giovanni Toraldo** — DevOps Engineer @ Hyland

    **_Shift left with performance testing_** - talk language 🇬🇧

- **Martino Fornasa** — Independent Consultant & Strategic Advisor | DevOps | Cloud Native | FinOps | Trainer

    **_Strategie incrementali per l'adozione di OpenTelemetry_** - talk language 🇮🇹

- **Giorgio Basile & Fabrizio Baldazzi** — Angular Frontend Developer & Solution Architect

    **_Superset, dashboard, embedding e dolori_** - talk language 🇮🇹

- **Katarina Zivkovic** — Brand & Content Strategist

    **_Synthetic Trust: AI, Security, and the Collapse of “Seeing Is Believing”_** - talk language 🇬🇧

- **Luca Di Vita** — Machine Learning Engineer @ Cy4Gate | Member @ GDG Pescara | Co-Founder @ Python Pescara

    **_Tutti a bordo delle Neural ODE!_** - talk language 🇮🇹

- **Christian Liebel** — Member of the W3C Technical Architecture Group (TAG)

    **_What’s New in Web? 2026 Edition_** - talk language 🇬🇧

<!-- END -->

Finally, if you love what we're doing, please share this amazing lineup with your friends and colleagues ❤️.

<a role="button" class="large width-fit margin-auto" target="_blank" href="https://www.eventbrite.it/e/devfest-pisa-2026-tickets-1985159725177">
    Get your Ticket
</a>

<a role="button" class="large width-fit margin-auto"  target="_blank" href="https://forms.gle/sE1of58ypRpYrw1C7">
    Fill out the form to become a Helper
</a>

See you soon at DevFest Pisa 2026 on April 18th! 🎊

DevFest Pisa Team 💻❤️

**_#DevFestPisa #GDG #TechCommunity #Developers #IWD #WTM #GDGPisa_**
